...
public class GamePanel extends JPanel implements KeyListener, ActionListener {
    ...
    private int level = 1; // **Added level variable**
    private int lastScoreIncrease = 0; // **Track last score increase**

    public GamePanel() {
        brickLayout = new BrickManager(6, 10, level); // **Pass level to BrickManager**
        addKeyListener(this);
        setFocusable(true);
    ...
    public void paint(Graphics g) {
        ...
        // Level Display
        g.drawString("Level: " + level, 50, 30); // **Display level**
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (isPlaying) {
                        ...
                        if (ballRect.intersects(brickRect)) {
                            if (brickLayout.bricks[i][j] != -1) { // Not an obstacle
                                ...
                                if (brickLayout.bricks[i][j] == 0) {
                                    ...
                                    if (currentScore % 50 == 0 && currentScore > lastScoreIncrease) {
                                        ballXSpeed *= 2; // Double the horizontal speed
                                        ballYSpeed *= 2; // Double the vertical speed
                                        lastScoreIncrease = currentScore;
                                    }
                                }
                            }
            ...
            // Increase level and reset bricks when all bricks are cleared
            if (bricksRemaining <= 0 && level < 5) {
                level++;
                brickLayout = new BrickManager(6, 10, level);
                bricksRemaining = 6 * 10;
                lastScoreIncrease = currentScore; // Reset last score increase
            }
        }
        repaint();
    }
